/* Motor de regras — Jogo de Cartas Minerais — v7 (Atualizado com Balanceamento, P&D e Simulador)
...
(function (raiz) {
  ...
  const PADRAO = {
    ...
    maoSegundo: 4,           // NERF 1: Caiu de 5 para 4. (No HS é 3 vs 4)
    fichaSegundo: [1],       // NERF 2: A "Moeda" agora dá apenas +1 de energia no turno 1, e não +2 e depois +1.
    baralhoSegundoExtra: 0,  // NERF 3: Caiu de 2 para 0. O baralho tem o mesmo tamanho exato do J1.
    // -------------------------------
    ...
    // Regra 3 (Refino Agressivo): O ato de refinar "cansa" a carta.
    ...
    // --- INTERRUPTORES DE TESTE (P&D E COMBOS) ---
    // Regra 2 (Refino Tático): Se true, atacar primeiro impede refinar a mesma carta no mesmo turno
    atacarImpedeRefinar: false, 
    
    // Regra 4 (Refino Surpresa): Se true, refinar a carta na mão proíbe de jogá-la no mesmo turno
    refinoMaoImpedeJogar: false, 
    
    // Regra 1 (Investida da Liga): true = ataca no mesmo turno que é formada, false = Enjoo de invocação
    ligaEntraPronta: true,      
    
    custoExtraRefinoMao: 0,     // Adiciona custo (ex: +1, +2) ao refinar direto da mão

    /* Ideia do Thomas, 10/08: a exaustao consome primeiro as barreiras e so
       depois a reserva. Em 0 o motor se comporta igual ao engine.js. */
    exaustaoComeBarreira: 0,

    /* Quantos refinos um jogador pode fazer por turno. 0 = sem limite, que
       e como o jogo sempre funcionou.

       Por que a chave existe: hoje o bot refina duas ou mais vezes no mesmo
       turno em so 3,7% dos turnos, o que faria a regra parecer inofensiva.
       Mas o bot escolhe uma acao por vez, sem planejar o turno. Uma pessoa
       com 9 de energia no fim da partida, refino custando 3 e tres cartas
       refinaveis na mesa, faz TRES refinos de uma vez e ergue um paredao.
       O bot nao procura essa jogada; a pessoa acha na terceira partida.

       Com limite 1, refinar vira uma decisao por turno — QUAL carta eu
       promovo hoje — em vez de gasto de energia acumulada. */
    refinoMaxPorTurno: 0
  };
  ...
  /* Custo de refinar uma carta. Suporta cálculo diferenciado se estiver na mão. */
  function custoRefino(est, dados, c, naMao = false) {
    if (c.nivel !== NIVEL.CONCENTRADO) return null;
    const b = cartaBase(dados, c.cartaId);
    
    let custoBase;
    if (b && b.refinoCusto != null && b.refinoCusto !== '') {
      custoBase = Number(b.refinoCusto);
    } else {
      custoBase = Math.max(est.cfg.refinoMin, Math.ceil(c.custo / est.cfg.refinoDivisor));
    }

    // Aplica o interruptor de custo extra para refinar direto da mão
    if (naMao && est.cfg.custoExtraRefinoMao > 0) {
      custoBase += est.cfg.custoExtraRefinoMao;
    }
  ...
  function novoJogo(dados, semente, cfg) {
    ...
    function instanciar(cartaId) {
      ...
      return {
        ...
        refinadaNoTurno: null,  // MEMÓRIA PARA A REGRA 4 (P&D na mão)
  ...
  function iniciarTurno(est, dados) {
    ...
    est.turno++;
    j.refinosNoTurno = 0;   // ver cfg.refinoMaxPorTurno
    ...
    if (j.baralho.length) {
      ...
      aplicarExaustao(est, j, j.exaustao);
      checarFim(est);
    }
  }

  /* ------------------------------------------------------------------
     EXAUSTAO QUE COME BARREIRA PRIMEIRO   (cfg.exaustaoComeBarreira)

     Ideia do Thomas: quando a jazida acaba, o que se consome primeiro nao
     e a saude do pais, sao as reservas que ele blindou. So depois de
     queimar as barreiras e que a Forca Geopolitica comeca a cair.

     Regra, em uma frase: o dano da exaustao gasta as barreiras uma a uma,
     da MENOR dureza para a maior, e o que sobrar passa para a reserva.

     Comer a menor primeiro e de proposito: o muro grande sobrevive mais
     tempo, entao o tabuleiro se desfaz aos poucos em vez de sumir de uma
     vez. Sacrificar o mais barato tambem e o que um jogador escolheria.

     Com a chave desligada (padrao), o comportamento e identico ao
     engine.js: o dano vai direto para a reserva. Isso e o que permite
     medir A com B sem trocar de motor.
     ------------------------------------------------------------------ */
  function aplicarExaustao(est, j, dano) {
    if (!est.cfg.exaustaoComeBarreira) {
      j.reserva -= dano;
      est.log.push('Exaustão: -' + dano);
      return;
    }
    let resto = dano;
    const comidas = [];
    while (resto > 0) {
      const muros = j.campo.filter(function (m) { return m.barreira && m.defesa > 0; });
      if (!muros.length) break;
      muros.sort(function (a, b) { return a.defesa - b.defesa; });
      const alvo = muros[0];
      const absorve = Math.min(resto, alvo.defesa);
      alvo.defesa -= absorve;
      resto -= absorve;
      if (alvo.defesa <= 0) {
        j.campo = j.campo.filter(function (m) { return m !== alvo; });
        comidas.push(nomeCurto(alvo.nome));
      }
    }
    if (comidas.length) {
      est.log.push('Exaustão consumiu a reserva blindada: ' + comidas.join(', '));
    }
    if (resto > 0) {
      j.reserva -= resto;
      est.log.push('Exaustão: -' + resto);
    } else if (!comidas.length) {
      est.log.push('Exaustão: -' + dano + ' absorvido pela barreira');
    }
  }
  ...
  function refinaveis(est, dados) {
    ...
    /* Se ja bateu o limite do turno, a lista vem vazia. Isto NAO e detalhe:
       se `refinaveis` oferecesse o que `refinar` recusa, a interface
       acenderia botao que nao funciona e o bot gastaria acao a toa. Foi
       exatamente o bug que ja aconteceu entre ligasDisponiveis e fundir. */
    if (est.cfg.refinoMaxPorTurno &&
        (j.refinosNoTurno || 0) >= est.cfg.refinoMaxPorTurno) return out;
    ...
    j.campo.forEach(function (c) {
      ...
      // REGRA 3: Refino Agressivo
      if (est.cfg.refinoGastaAcao && !c.pronta) return;   
      
      // REGRA 2: Refino Tático
      if (est.cfg.atacarImpedeRefinar && !c.pronta) return; 
      
      const custo = custoRefino(est, dados, c, false);
      if (custo <= j.energia) {
    ...
    j.mao.forEach(function (c) {
      if (c.nivel !== NIVEL.CONCENTRADO) return;
      const custo = custoRefino(est, dados, c, true);
      if (custo <= j.energia) {
  ...
  function refinar(est, dados, uid) {
    ...
    /* Limite de refinos por turno. Em 0 nao ha limite, que e o padrao. */
    if (est.cfg.refinoMaxPorTurno &&
        (j.refinosNoTurno || 0) >= est.cfg.refinoMaxPorTurno) {
      return erro(est.cfg.refinoMaxPorTurno === 1
        ? 'Só dá para refinar uma vez por turno.'
        : 'Já refinou ' + est.cfg.refinoMaxPorTurno + ' vezes neste turno.');
    }
    ...
    if (noCampo) {
      ...
      // REGRA 2
      if (est.cfg.atacarImpedeRefinar && !c.pronta) {
        return erro(nomeCurto(c.nome) + ' já atacou e não pode ser refinado agora.');
      }
    }

    const custo = custoRefino(est, dados, c, !noCampo);
    ...
    // MARCA O TURNO DE REFINO PARA A REGRA 4
    c.refinadaNoTurno = est.turno;
    j.refinosNoTurno = (j.refinosNoTurno || 0) + 1;
    ...
    for (const nome of lg.cartas) {
      ...
      if (m) {
        ...
        if (est.cfg.exigirRefinoParaLiga && !est.cfg.refinoGratisNaMao && m.nivel === NIVEL.CONCENTRADO) {
           pedagio = custoRefino(est, dados, m, true);
        }
  ...
  function fundir(est, dados, nomeLiga) {
    ...
    const nova = {
      ...
      estreouNoTurno: est.turno,
      
      // REGRA 1 (Investida da Liga)
      pronta: est.cfg.ligaEntraPronta !== undefined ? est.cfg.ligaEntraPronta : true,
  ...
  function jogar(est, dados, uid) {
    ...
    // REGRA 4 (Refino Surpresa bloqueado)
    if (est.cfg.refinoMaoImpedeJogar && c.refinadaNoTurno === est.turno) {
      return erro('Carta sofreu P&D neste turno e não pode ser extraída agora.');
    }
  ...
  function atacar(est, dados, uidOrigem, alvo) {
    ...
    if (alvo === 'reserva') {
      ...
      op.reserva -= a.ataque;
      a.pronta = false; // Ao atacar, perde a prontidão
      ...
      a.pronta = false; // O ataque consome a ação da carta.
  ...
  /* CLONAR O ESTADO — a peça que faltava para o bot pensar.

     O `Motor.aplicar` altera o estado no lugar. Um bot que pergunta "e se
     eu fizesse isso?" precisa de uma CÓPIA para experimentar, senão
     perguntar é jogar.

     Veio do `engine2.js`, que era o `engine.js` mais esta função. Trazida
     para cá em 11/08 para não haver dois motores — dois arquivos com o
     mesmo `versao: 11` é a armadilha em que se cai uma vez e se perde um
     dia inteiro.

     Cópia RASA onde dá e profunda só onde precisa: `cfg` e `dados` não
     mudam durante a partida, então são compartilhados; carta e campo são
     copiados porque o combate mexe neles.

     Atenção: o clone NÃO leva `log` nem `acoes`. É de propósito — quem
     clona está simulando, não jogando, e não deve poluir o registro. */
  function clonarEstado(est) {
    const cloneJogadores = est.jogadores.map(function(j) {
      return {
        reserva: j.reserva,
        energia: j.energia,
        energiaMax: j.energiaMax,
        baralho: j.baralho.slice(),
        mao: j.mao.map(function(c) { 
          return Object.assign({}, c, { partes: c.partes ? c.partes.slice() : [] }); 
        }),
        campo: j.campo.map(function(c) { 
          return Object.assign({}, c, { partes: c.partes ? c.partes.slice() : [] }); 
        }),
        exaustao: j.exaustao,
        refinosNoTurno: j.refinosNoTurno,
        turnosJogados: j.turnosJogados
      };
    });
    ...
    return {
      ...
      vez: est.vez,
      turno: est.turno,
      jogadores: cloneJogadores,
      log: [], 
      acoes: [], 
      fim: est.fim,
      _contador: { n: est._contador.n },
      _instanciar: est._instanciar
    };
  }
